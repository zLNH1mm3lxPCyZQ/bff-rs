...
pub fn load_obj(file: &str) -> Result<PolygonSoup, String> {
    ...
    if indices.iter().any(|index| *index > n_vertices) {
        return Err("Some faces indices are out of bounds with the vertices array.".to_string());
    }

    Ok(PolygonSoup { vertices, indices })
}

fn main() -> Result<(), String> {
    let soup = load_obj("resources/hemisphere.obj")?;
